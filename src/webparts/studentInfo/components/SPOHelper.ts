 ...
 * @param options
 */
const BaseClient = (url = "", options?) => {
    let { headers = {}, method = "GET", body = undefined } = isObject(options)
        ? options
        : {};
...
const BaseRequest = (url = "", options) => {
    return BaseClient(url, options).then(r => r);
};

/**
 * Get JSON response with Base HttpClient which is wrapper of fetch.
 * @param options
 */
const GetJson = (url = "", options) => {
    return BaseClient(url, options).then(r => r.json());
};
...
const SPOHelper = () => {
    let rootUrl = "";
    let isListExist = listName => {
        return BaseClient(`${rootUrl}/_api/Lists/GetByTitle(${listName})`).then(
            r => r.ok
        );
    };
    let isSiteExist = siteName => {
        let reqUrl = rootUrl.toLowerCase().split("/sites/")[0];
        return BaseClient(`${reqUrl}/sites/${siteName}/_api/web/id`).then(
            r => r.ok
        );
    };
};
